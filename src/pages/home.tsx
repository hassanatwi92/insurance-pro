...
import "react-datepicker/dist/react-datepicker.css"; // 👈 Darouri kermel el design
...
function formatDateDMY(dateStr?: string): string {
  ...
  // Iza el teri5 aslan fih "-" (metel yyyy-mm-dd)
  ...
  return `${d}-${m}-${y}`; // Hayda el format li baddak ye
}
...
export default function Home({ username, page, setPage, isCompaniesPage }: { username: string; page: Page; setPage: (p: Page) => void; isCompaniesPage?: boolean }) {
 ...
 const [form, setForm] = useState({
  ...
  const [formInstallments, setFormInstallments] = useState<{ date: string; method: string; receipt_num: string; amount: string }[]>([
    { date: "", method: "", receipt_num: "", amount: "" },
    { date: "", method: "", receipt_num: "", amount: "" },
  ]);

const storageKey = isCompaniesPage ? `insuranceData_companies_${username}` : `insuranceData_${username}`;
const companyListKey = `insuranceCompanyList_${username}`;
const policyTypeKey = `insurancePolicyTypes_${username}`;
const clientNamesKey = `insuranceClientNames_${username}`;
const brokerListKey = `insuranceBrokers_${username}`;
...
useEffect(() => {
  ...
  if (saved) {
    try { setPolicies(JSON.parse(saved)); }
    catch { setPolicies([]); }
  }

  const savedCo = localStorage.getItem(companyListKey);
  if (savedCo) {
    try { setCompanyList(JSON.parse(savedCo)); }
    catch { setCompanyList([]); }
  }

  const savedClients = localStorage.getItem(clientNamesKey);
  if (savedClients) {
    try { setAllClientNames(JSON.parse(savedClients)); }
    catch { setAllClientNames([]); }
  }

  const savedBrokers = localStorage.getItem(brokerListKey);
if (savedBrokers) {
  try { setBrokers(JSON.parse(savedBrokers)); }
  catch { setBrokers([]); }
}

const companyStorageKey = `insuranceData_companies_${username}`;
const savedCompanyPolicies = localStorage.getItem(companyStorageKey);
if (savedCompanyPolicies) {
  try { setCompanyPolicies(JSON.parse(savedCompanyPolicies)); }
...
useEffect(() => {
  const savedTypes = localStorage.getItem(policyTypeKey);
  if (savedTypes) {
    try { setPolicyTypes(JSON.parse(savedTypes)); }
...
function confirmPassword() {
  ...
  const users = JSON.parse(localStorage.getItem("insurance_users") || "[]");
  ...
  function saveCompanyList(list: string[]) {
    localStorage.setItem(companyListKey, JSON.stringify(list));
    setCompanyList(list);
  }

  function savePolicyTypes(list: string[]) {
  localStorage.setItem(policyTypeKey, JSON.stringify(list));
  setPolicyTypes(list);
}

function saveClientNames(list: string[]) {
  localStorage.setItem(clientNamesKey, JSON.stringify(list));
  setAllClientNames(list);
}


function saveBrokers(list: BrokerItem[]) {
  localStorage.setItem(brokerListKey, JSON.stringify(list));
  setBrokers(list);
}
...
function addBroker() {
  ...
  saveBrokers([...brokers, { name, code }]);
  setNewBrokerName("");
  setNewBrokerCode("");
}

function deleteBroker(code: string) {
  saveBrokers(brokers.filter((b) => b.code !== code));
}
...
function addPolicyType() {
  ...
  savePolicyTypes([...policyTypes, name]);
  setNewPolicyType("");
}

function deletePolicyType(name: string) {
  savePolicyTypes(policyTypes.filter((t) => t !== name));
}

  function addCompanyToList() {
    const name = newCompanyName.trim();
    if (!name) return;
    if (companyList.includes(name)) { setNewCompanyName(""); return; }
    saveCompanyList([...companyList, name]);
    setNewCompanyName("");
  }

  function deleteCompanyFromList(name: string) {
    saveCompanyList(companyList.filter((c) => c !== name));
  }

  function saveData(data: Policy[]) {
  ...
  function updateInstallmentsCount(count: string) {
    ...
    const next = Array.from({ length: n }, (_, i) => current[i] || { date: "", method: "", receipt_num: "", amount: "" });
  ...
  function addPolicy() {
    ...
    if (!form.policy_num || !form.client_name) { showMessage("يرجى ملء رقم البوليصة واسم العميل", "warning"); return; }
...
setForm({
  ...
  installments_count: "2"
    ...
    setClientSearch("");
    setFormInstallments([{ date: "", method: "", receipt_num: "", amount: "" }, { date: "", method: "", receipt_num: "", amount: "" }]);
    setShowDialog(false);
    showMessage(`✅ تمت إضافة البوليصة! الربح: $${profit.toFixed(2)}`, "success");
  }

  function togglePaidCompany(id: number) {
    saveData(policies.map((p) => p.id === id ? { ...p, paid_company: !p.paid_company } : p));
  }

function toggleInstallment(policyId: number, instIndex: number) {
  saveData(
    policies.map((p) => {
...
function updateInstallmentField(
  ...
  const patchPolicy = (p: Policy) => {
    ...
    const updated = p.installments.map((ins) => {
      ...
      if (newIns.payment_date && newIns.method) {
        newIns.paid = true;
      } else {
        newIns.paid = false;
      }
  ...
  saveData(newPolicies);
  setPaymentModal((prev) => (prev ? (patchPolicy(prev) as Policy) : prev));
}
  ...
  function deletePolicy(id: number) {
    if (window.confirm("هل أنت متأكد من حذف هذه البوليصة؟")) {
      saveData(policies.filter((p) => p.id !== id));
  ...
  if (window.confirm("هل تريد حذف جميع البيانات؟")) {
    localStorage.removeItem(storageKey);
      setPolicies([]);
      showMessage("🗑️ تم مسح جميع البيانات!", "warning");
    }
  }
...
function getPrintSearchLabel() {
  ...
  if (searchField === "client_name") {
    return `اسم العميل: ${value}`;
  }

  if (searchField === "policy_num") {
    return `رقم البوليصة: ${value}`;
  }

  if (searchField === "insurance_company") {
    return `شركة التأمين: ${value}`;
  }

  if (searchField === "policy_type") {
    return `نوع البوليصة: ${value}`;
  }

  if (searchField === "broker") {
    const broker = brokers.find(
      (b) => b.code.toLowerCase() === value.toLowerCase()
    );

    if (broker) {
      return `الوسيط: ${broker.name} / كود: ${broker.code}`;
    }

    return `كود الوسيط: ${value}`;
  }
...
function getSearchFieldLabel(value: SearchField) {
  if (value === "client_name") return "اسم العميل";
  if (value === "policy_num") return "رقم البوليصة";
  if (value === "insurance_company") return "شركة التأمين";
  if (value === "policy_type") return "نوع البوليصة";
  if (value === "broker") return "كود الوسيط";
  return "الكل";
}
  ...
  const fieldStyle: React.CSSProperties = { width: "100%", padding: "10px 14px", border: "2px solid #e0e0e0", borderRadius: 10, fontSize: 14, outline: "none", direction: "rtl", boxSizing: "border-box", fontFamily: "inherit" };
  const labelStyle: React.CSSProperties = { display: "block", marginBottom: 4, fontWeight: "bold", color: "#555", fontSize: "0.88em" };
...
<Btn
  onClick={() => setShowPolicyTypeManager(true)}
  color="linear-gradient(45deg,#8E24AA,#6A1B9A)"
>
...
<Btn
  ...
  color="linear-gradient(45deg,#FB8C00,#EF6C00)"
>
  👔 إدارة الوسطاء ({brokers.length})
        ...
        {displayed.length > 0 && (
          <>
            ...
            <div
  id="print-table"
  className={isCompaniesPage ? "companies-print" : "clients-print"}
...
<div id="print-summary" style={{ display: "none" }}>
  ...
  {paidFilter !== "all" && (
    <div className="print-meta-line">
      📊 الفلترة: {getPaidFilterLabel(paidFilter)}
    </div>
  )}
</div>
...
{displayed.flatMap((p, i) => {
    ...
    p.installments.forEach((ins, idx) => {
      ...
      rows.push(
        <tr key={`${p.id}-${idx}`} style={{ background: (i + idx) % 2 === 0 ? "#fff" : "#f9f9f9" }}>
          ...
          <td style={tdStyle}>
            {p.policy_type
              ? <span style={{ background: "#E3F2FD", color: "#1565C0", borderRadius: 5, padding: "2px 7px", fontSize: "0.8em", fontWeight: "bold" }}>{p.policy_type}</span>
              : <span style={{ color: "#bbb" }}>—</span>}
          </td>

          <td style={tdStyle}>
            {p.insurance_company
              ? <span style={{ background: "#F3E5F5", color: "#6A1B9A", borderRadius: 5, padding: "2px 7px", fontSize: "0.8em", fontWeight: "bold" }}>{p.insurance_company}</span>
              : <span style={{ color: "#bbb" }}>—</span>}
          </td>
  ...
  <DatePicker
    ...
    // Hon mna3mol chwayye styling kermel ma y-kharrib el table
  ...
  {ins.method === "ملغى" ? (
    ...
    <input
      ...
      value={ins.method || ""}
      onChange={(e) =>
        updateInstallmentField(p.id, ins.index, "method", e.target.value)
      }
    ...
    />
          ...
          <td style={tdStyle}>
            <input
              type="text"
              value={ins.receipt_num || ""}
              onChange={(e) =>
                updateInstallmentField(p.id, ins.index, "receipt_num", e.target.value)
              }
  ...
  .map((i, idx) => ({
    ...
    if (newInstallments.length > 0) {
      return {
        ...
        installments_count: newInstallments.length
      };
    }
    ...
    return {
      ...
      installments: [
        {
          index: 1,
          due_date: ins.due_date, // نحافظ على تاريخ الاستحقاق
          payment_date: today,    // تاريخ اليوم
          method: "ملغى",
          receipt_num: "",
          amount: "0",
          paid: true
      ...
      onClick={() => {
        ...
        const updated = policies.map(pol => {
          ...
          return {
            ...
            sell_price: 0,
            profit: 0,
            cash_date: today,          // 📅 تاريخ اليوم
            cash_method: "ملغى",       // ❌ ملغى
            cash_receipt_num: "",
            paid_company: true         // متل الأقساط الملغية
          };
        });
...
{/* صف المجموع للطباعة - صفحة الزبائن */}
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {(["cash", "installment"] as const).map((t) => (
                  <button key={t} type="button" onClick={() => setForm({ ...form, client_payment_type: t })} style={{ flex: 1, padding: "10px", border: "none", borderRadius: 10, fontSize: 14, fontWeight: "bold", cursor: "pointer", fontFamily: "inherit", background: form.client_payment_type === t ? (t === "cash" ? "#4CAF50" : "#2196F3") : "#f0f0f0", color: form.client_payment_type === t ? "white" : "#555" }}>
                    {t === "cash" ? "💵 نقدي" : "📅 تقسيط"}
                  </button>
                ))}
              </div>
...
{form.client_payment_type === "cash" && (
      ...
      <div>
        ...
        <DatePicker
          selected={form.cash_date ? new Date(form.cash_date) : null}
          onChange={(date: Date | null) => {
            if (date) {
              const y = date.getFullYear();
              const m = String(date.getMonth() + 1).padStart(2, '0');
          ...
          dateFormat="dd-MM-yyyy"
          className="my-custom-datepicker" // Fik t-zabbit el CSS taba3a
          customInput={
    <input
      style={{
        ...fieldStyle,
...
<DatePicker
  ...
  onChange={(date) => {
    if (date) {
      ...
      next[i] = {
        ...next[i],
        date: `${y}-${m}-${d}`, // نفس تخزين النقدي
      };
      setFormInstallments(next);
    }
  }}
  dateFormat="dd-MM-yyyy" // 👈 نفس الشكل
  placeholderText="dd-mm-yyyy"
  ...
  >
    ...
    >
      ...
      <input
        ...
        onChange={(e) =>
          setEditModal({ ...editModal, client_name: e.target.value })
        }
        style={inputStyle}
      />
      ...
      <input
        type="text"
        value={editModal.policy_num}
        onChange={(e) =>
          setEditModal({ ...editModal, policy_num: e.target.value })
        }
        style={inputStyle}
      />
      ...
      <input
        type="text"
        value={editModal.policy_type || ""}
        onChange={(e) =>
          setEditModal({ ...editModal, policy_type: e.target.value })
        }
        style={inputStyle}
      />
      ...
      <input
        type="text"
        value={editModal.insurance_company || ""}
        onChange={(e) =>
          setEditModal({ ...editModal, insurance_company: e.target.value })
        }
        style={inputStyle}
      />
...
<label style={labelStyle}>👔 Broker</label>
<select
  value={editModal.broker_code || ""}
  onChange={(e) => {
    const selectedBroker = brokers.find((b) => b.code === e.target.value);
    setEditModal({
      ...editModal,
      broker_code: selectedBroker?.code || "",
      broker_name: selectedBroker?.name || ""
...
>
  ...
  {brokers.map((b) => (
    <option key={b.code} value={b.code}>
      ...
      <input
        type="number"
        value={editModal.buy_price}
        onChange={(e) =>
          setEditModal({
            ...editModal,
            buy_price: parseFloat(e.target.value) || 0,
            profit:
              editModal.sell_price - (parseFloat(e.target.value) || 0)
          })
        }
        style={inputStyle}
      />
      ...
      <input
        type="number"
        value={editModal.sell_price}
        onChange={(e) =>
          setEditModal({
            ...editModal,
            sell_price: parseFloat(e.target.value) || 0,
            profit:
              (parseFloat(e.target.value) || 0) - editModal.buy_price
          })
        }
        style={inputStyle}
      />
      ...
      <div style={{
        ...
        الربح: ${editModal.profit.toFixed(2)}
      </div>
...
>
  ...
  <div style={{ maxHeight: 200, overflowY: "auto" }}>
    {editModal.installments?.map((ins, i) => (
      ...
      >
        <div style={{ fontSize: "0.8em", marginBottom: 5 }}>
          قسط {ins.index}
        </div>
  ...
  setEditModal({
    ...
    installments: updated
          ...
          <input
            ...
            onChange={(e) => {
              const updated = [...editModal.installments];
              updated[i].receipt_num = e.target.value;
              setEditModal({ ...editModal, installments: updated });
            }}
            placeholder="رقم الواصل"
          />
                ...
                <div key={ins.index} style={{ marginBottom: 10, background: ins.paid ? "#f0fff0" : "#fff8f8", borderRadius: 12, border: `1px solid ${ins.paid ? "#c8e6c9" : "#ffcdd2"}`, padding: "10px 12px" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6 }}>
                    <div>
                      ...
                      <input type="date" value={ins.payment_date || ""} onChange={(e) => updateInstallmentField(paymentModal.id, ins.index, "payment_date", e.target.value)} style={{ width: "100%", padding: "6px 6px", border: "1px solid #ddd", borderRadius: 7, fontSize: 11, direction: "rtl", boxSizing: "border-box", fontFamily: "inherit" }} />
                    </div>
                    <div>
                      <div style={{ fontSize: "0.7em", color: "#888", marginBottom: 3 }}>🏦 الطريقة</div>
                      <input type="text" value={ins.method || ""} onChange={(e) => updateInstallmentField(paymentModal.id, ins.index, "method", e.target.value)} placeholder="كاش، شيك..." style={{ width: "100%", padding: "6px 6px", border: "1px solid #ddd", borderRadius: 7, fontSize: 11, direction: "rtl", boxSizing: "border-box", fontFamily: "inherit" }} />
                    </div>
                    <div>
                      <div style={{ fontSize: "0.7em", color: "#888", marginBottom: 3 }}>🧾 رقم الواصل</div>
                      <input type="text" value={ins.receipt_num || ""} onChange={(e) => updateInstallmentField(paymentModal.id, ins.index, "receipt_num", e.target.value)} placeholder="رقم الواصل" style={{ width: "100%", padding: "6px 6px", border: "1px solid #ddd", borderRadius: 7, fontSize: 11, direction: "rtl", boxSizing: "border-box", fontFamily: "inherit" }} />
                    </div>
                    <div>
                      <div style={{ fontSize: "0.7em", color: "#888", marginBottom: 3 }}>💵 المبلغ</div>
                      <input type="number" value={ins.amount || ""} onChange={(e) => updateInstallmentField(paymentModal.id, ins.index, "amount", e.target.value)} placeholder="0.00" style={{ width: "100%", padding: "6px 6px", border: "1px solid #ddd", borderRadius: 7, fontSize: 11, direction: "rtl", boxSizing: "border-box", fontFamily: "inherit" }} />
                    </div>
                  </div>
                </div>
              ))}
...
<style>{`
  @media print {
    ...
    #print-summary .print-meta-line {
      margin: 5px 0;
      color: #444;
    }

    /* صفحة الزبائن فقط: اخفاء الشراء والبيع */
    ...
    /* اخفاء صف المجموع العادي فقط بصفحة الزبائن */
    .clients-print .normal-total-row {
      display: none !important;
    }

    /* اظهار صف مجموع خاص بالطباعة فقط بصفحة الزبائن */
    ...
    /* اخفاء عمود Broker فقط عند الطباعة */
...
`}</style>
          ...
          <div dir="rtl" style={{ background: "white", borderRadius: 20, padding: 28, width: "100%", maxWidth: 420, boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && addCompanyToList()}
                placeholder="اسم الشركة الجديدة..."
                style={{ flex: 1, padding: "10px 14px", border: "2px solid #e0e0e0", borderRadius: 10, fontSize: 14, direction: "rtl", outline: "none", fontFamily: "inherit" }}
              />
            ...
            <div style={{ maxHeight: 300, overflowY: "auto" }}>
              {companyList.length === 0 ? (
                <div style={{ textAlign: "center", color: "#bbb", padding: 20, fontSize: "0.9em" }}>لا توجد شركات بعد</div>
              ) : (
 ...
 {showPolicyTypeManager && (
  <div
    ...
    onClick={(e) => {
      if (e.target === e.currentTarget) setShowPolicyTypeManager(false);
    }}
  >
    <div
      dir="rtl"
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
        <input
          ...
          placeholder="نوع البوليصة..."
          style={{ flex: 1, padding: 10 }}
        />
        <button onClick={addPolicyType}>➕</button>
      </div>
      ...
      <div style={{ maxHeight: 300, overflowY: "auto" }}>
        {policyTypes.map((t) => (
          ...
          >
            <span>{t}</span>
            <button onClick={() => deletePolicyType(t)}>🗑️</button>
          </div>
      ...
      <button
        onClick={() => setShowPolicyTypeManager(false)}
...
{showBrokerManager && (
  <div
    ...
    onClick={(e) => {
      if (e.target === e.currentTarget) setShowBrokerManager(false);
    }}
  >
    <div
      dir="rtl"
...
>
  ...
  >
    ➕ إضافة
        ...
        {brokers.length === 0 ? (
          ...
          brokers.map((b) => (
            ...
            >
              ...
              <button onClick={() => deleteBroker(b.code)}>🗑️</button>
            </div>
          ))
        )}
      </div>
      ...
      <button
        onClick={() => setShowBrokerManager(false)}
        style={{ width: "100%", marginTop: 15 }}
      >
        إغلاق
      </button>
